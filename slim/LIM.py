...
class LIM(object):
    ...
    def __init__(self, tau0_data, tau1_data=None, nelem_in_tau1=1,
                 fit_noise=False, max_neg_Qeval=5e10, h5file=None):
        """
        Parameters
        ----------
        tau0_data: ndarray
        ...
        self.L_eigs = np.log(eigvals(self.G_1))
    ...
    def _calc_Q(G, x0, tau=1, max_neg_evals=5):
        ...
        if num_neg > 0:
            ...
            if num_neg > max_neg_evals:
                ...
                logger.debug('More than {:d} negative eigenvalues of Q '
                                 'detected.  Consider further dimensional '
                                 'reduction.'.format(max_neg_evals))

            else:
                logger.info('Found {:d} modes with negative eigenvalues in' 
                            ' the noise covariance term, Q.'.format(num_neg))
    ...
    def noise_integration(self, t0_data, length, timesteps=1440,
        ...
        ndarray
            ...
            as input t0_data.
        ### Zilu Meng Attention: the return value length_out_arr[0] is all zeros.
        """
        ...
        # change to save time
        # 2024 May 18: Zilu Meng changed the loop to save time: The story is that max(i) is integration_steps - 1, meaning that i can be saved as the last step ((i = intergration_steps) % timesteps ==0 ), so the last several steps can be saved.
        for i in range(0, integration_steps - timesteps + 1):
            deterministic = (L @ state_1) * tdelta
            random = np.random.normal(size=(num_evals, nens))
            stochastic = Q_evec @ (np.sqrt(Q_eval * tdelta) * random)
            # print(deterministic.shape, stochastic.shape)
...
if __name__ == "__main__":
    ...
    pc = np.load("./pckk.npy")
    # np.save("pckk.npy", pc)
    cslim = LIM(pc.T, fit_noise=True)
    cslim.noise_integration(pc.T[[0]], 10)
    # cslim.cal_noise()
    # # res = cslim.forecast(pc.T[[0]], month0=1, length=10)
    # res = cslim.noise_intergral(pc.T[[0]], month0=1, length=10)
    # print(res.shape)
